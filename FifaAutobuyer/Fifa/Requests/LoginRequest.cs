...
namespace FifaAutobuyer.Fifa.Requests
{
    ...
    {
        public LoginRequest(FUTAccount account, FUTSession session, RequestPerMinuteManager rpmManager, RequestPerMinuteManager rpmManagerSearch, LoginMethod login) : base(account, session, rpmManager, rpmManagerSearch, login)
        {
        ...
        public async Task<LoginResponse> PerformRequestAsync()
        {
            try
            {
                ...
                //if (futCookies == null)
                //{
                //    UpdateStatistic("FUTCookies are null");
                //    futCookies = new FUTCookies();
                //    futCookies.EMail = FUTAccount.EMail;
                //}
                //else
                //{
                //    UpdateStatistic("Loading Cookies...");
                //    HttpClient.SetCookieContainer(futCookies.CookiesWeb);
                //}
                HttpClient.SetCookieContainer(CookieManager.GetCookieContainer(FUTAccount));
                ...
                if (!loggedIn.RequestMessage.RequestUri.AbsoluteUri.Contains("https://www.easports.com/de/fifa/ultimate-team/web-app/auth.html"))
                {
                ...
                //if (loggedInString.Contains("tfa-form-container") && loggedInString.Contains("oneTimeCode"))
                //{
                //    UpdateStatistic("Filling Google TwoFactor...");
                //    await Task.Delay(2000).ConfigureAwait(false);
                //    var tfAuthCode = QuickEAAuthenticator.GenerateAuthCode(FUTAccount.GoogleAuthCode);
                //    loggedIn = await FillInTwoFactorAsync(loggedIn.RequestMessage.RequestUri.AbsoluteUri, tfAuthCode).ConfigureAwait(false);
                //    loggedInString = await loggedIn.Content.ReadAsStringAsync().ConfigureAwait(false);
                //}
                ...
                if (!loggedIn.RequestMessage.RequestUri.AbsoluteUri.Contains("https://www.easports.com/de/fifa/ultimate-team/web-app/auth.html"))
                {
                    ...
                    resp.Message = "No redirect to web-app";
                    return resp;
                }
                UpdateStatistic("Grabbing EASW ID...");

                var bearerAuth = Regex.Match(loggedIn.RequestMessage.RequestUri.Fragment, "access_token=(.*?)&").Groups[1].Value;

                var easw = await GetEaswAsync(bearerAuth);
                ...
                if (string.IsNullOrEmpty(easwID))
                {
                    ...
                    CookieManager.DeleteCookieContainer(FUTAccount, true);
                ...
                var authCode = await GetAuthCodeAsync(bearerAuth);
                ...
                if (string.IsNullOrEmpty(authed?.sid))
                {
                    resp.Code = Models.FUTErrorCode.NoSessionID;
                    resp.Message = "No sid";
                    return resp;
        ...
        {
            var loginPage = await HttpClient.GetAsync("https://accounts.ea.com/connect/auth?prompt=login&accessToken=null&client_id=FIFA-18-WEBCLIENT&response_type=token&display=web2/login&locale=de_DE&redirect_uri=https://www.easports.com/de/fifa/ultimate-team/web-app/auth.html&scope=basic.identity+offline+signin").ConfigureAwait(false);
            return loginPage;
        }

        private async Task<bool> IsUserLoggedInAsync()
        {
            var loginPage = await HttpClient.GetAsync("https://accounts.ea.com/connect/auth?client_id=FIFA-18-WEBCLIENT&response_type=token&display=web2/login&locale=de_DE&redirect_uri=nucleus:rest&prompt=none&scope=basic.identity+offline+signin").ConfigureAwait(false);
            var loginPageString = await loginPage.Content.ReadAsStringAsync().ConfigureAwait(false);
            return !loginPageString.Contains("login_required");
        }
        ...
        {
            var loginPage = await HttpClient.GetAsync("https://accounts.ea.com/connect/auth?client_id=FOS-SERVER&redirect_uri=nucleus:rest&response_type=code&access_token=" + accessToken).ConfigureAwait(false);
            return await loginPage.Content.ReadAsStringAsync();
        }

        private async Task<AuthResponse> AuthAsync(string authCode)
        {
            var stringContent = new StringContent("{\"isReadOnly\":false,\"sku\":\"" + FUTPlatform.SKU + "\",\"clientVersion\":1,\"locale\":\"de-DE\",\"method\":\"authcode\",\"priorityLevel\":4,\"identification\":{\"authCode\":\"" + authCode + "\",\"redirectUrl\":\"nucleus:rest\"},\"nucleusPersonaId\":" + FUTSession.PersonaID + ",\"gameSku\":\"" + FUTPlatform.GameSKU +  "\"}");
        ...
        {
            CookieManager.SaveCookieContainer(FUTAccount, HttpClient.ClientHandler.CookieContainer);
        }
